...
;(function($, window, document, undefined) {
	...
	PageSlider.prototype.showSec = function(index, direction) {
		if ($('.page.current').length) $('.page.current,.page.next,.page.prev').css({
			'-webkit-transition': null,
			'-webkit-transform': null
		}).removeClass('current prev next');
		var cur, next, prev;
		var totalSec = $('.page').length;
		if (direction == 'next') {
		...
		var $curSec = $('.page' + cur);
		var $nextSec = $('.page' + next);
		var $prevSec = $('.page' + prev);

		$curSec.addClass('current');
		this.pageHeight = $('.page.current').height();
		...
		$curSec.addClass('ani').siblings('.page').removeClass('ani');
		//执行回调
		this.opt.callback && this.opt.callback(cur);
	};
	...
	PageSlider.prototype.touchStartHandler = function(event) {
		...
		that.index = $('.page').index($(this)) + 1;
		var touch = event.touches[0];
		that.distance = 0;
		that.startY = touch.clientY;
		that.curPagePos = $(this).offset().top;
		that.nextPagePos = $('.page.next').length && $('.next').offset().top;
		that.prevPagePos = $('.page.prev').length && $('.prev').offset().top;
	...
	PageSlider.prototype.touchMoveHandler = function(event) {
		...
		if (!that.opt.elastic && ((that.index === 1 && that.distance > 0) || (that.index === $('.page').length && that.distance < 0))) {
			event.preventDefault();
			return;
		}
		...
		$('.page.next').css('-webkit-transform', that.motion(that.nextPageTop));
		$('.page.prev').css('-webkit-transform', that.motion(that.prevPageTop));
		event.preventDefault();
	};
	...
	PageSlider.prototype.touchEndHandler = function(event) {
		...
		$('.page.current,.page.next,.page.prev').css('-webkit-transition', '-webkit-transform ' + that.opt.duration + 'ms linear');
		if ((that.distance < -that.opt.range && $('.page.next').length) || (that.distance > that.opt.range && $('.prev').length)) {
			var next = !!(that.distance < -that.opt.range);
			$(this).css('-webkit-transform', that.motion((next ? (-$(this).height()) : $(this).height())));
			$(next ? '.page.next' : '.page.prev').css('-webkit-transform', that.motion(0));
			...
			$('.page.prev').css('-webkit-transform', that.motion(-that.pageHeight));
			$('.page.next').css('-webkit-transform', that.motion(that.pageHeight));
			that.timer = setTimeout(function() {
				$('.page.current,.page.next,.page.prev').css({
					'-webkit-transition': null
				});
				clearTimeout(that.timer);
				that.timer = null;
	...
	PageSlider.prototype.init = function(optOrIndex){
		...
		if (!that.inited) {
			$('.page').off('touchstart').on('touchstart', {
				'that': that
